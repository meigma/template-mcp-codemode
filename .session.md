...
- Lifecycle journal writes happen in the journal root, not in implementation
  worktrees.
- Before writing session state, lifecycle skills run a journal sync transaction:
  inspect journal worktree status, commit dirty active-session `NOTES.md`
  checkpoints if those are the only changes, pull/rebase, push any checkpoint
  commits, write the requested mutation, commit it, and push the journal branch.
  Dirty non-note files or mixed lifecycle artifacts must stop the transaction
  until clarified. On rejected push, fetch/rebase once and retry once; if
  conflicts remain, stop and surface them.
